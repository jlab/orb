...
def get_environments(fp_orb_basedir: str, settings) -> [str]:
    ...
    environments = [
        basename(fp) for fp in glob(join(fp_orb_basedir, "*")) if basename(fp) not in settings["skip_environments"]
    ]
    # sort environments according to YAML occurrence
    environments = sorted(
        environments,
        key=lambda x: (
            list(settings["labels"]["environments"].keys()).index(x)
            if x in settings["labels"]["environments"]
            else float("inf")
        ),
    )
...
def getdata_recovery(fp_orb_basedir: str, settings, detail_view=False, verbose=True) -> pd.DataFrame:
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for contig recovery plot"):
        # load orb data
        orb = pd.read_csv(
            join(fp_orb_basedir, environment, "mergedresults", "%s_all_scores.tsv" % environment), sep="\t", index_col=0
        )
        # TODO remove
        ...
        trueBlocks = (
            pd.read_csv(
                join(fp_orb_basedir, environment, "mergedataframessummaries", "%s_all_contigs.tsv" % environment),
                sep="\t",
                index_col=0,
            )
            .loc["count", ["Blocks"]]
        ...
        orb.loc["missed blocks", :] = (
            trueBlocks
            - orb.loc[[c["label"] for _, c in settings["contig_classes"].items() if c["goodblockcount"]], :].sum()
        )
        if detail_view:
            ...
            orb = orb[
                orb.loc[[c["label"] for _, c in settings["contig_classes"].items() if c["class"] == "good"]]
                .sum()
            ...
            for key, value in settings["simplelabels"].items():
                ...
                if display_name != "":
                    s = orb.loc[
                        [c["label"] for _, c in settings["contig_classes"].items() if c["simplelabel"] == key], :
                    ].sum()
                    s.name = display_name
                    df = pd.concat([df, s.to_frame().T])
            orb = df
            orb = orb[
                orb.loc[[c["label"] for _, c in settings["simplelabels"].items() if c["class"] == "good"]]
                .sum()
                .sort_values(ascending=True)
                .index
            ]
...
def get_recovered_contigs(fp_orb_basedir: str, settings, verbose=True):
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for gene recovery plot"):
        ...
        for fp_category in glob(
            join(fp_orb_basedir, environment, "minimap2classification", "*_minimap2_categories.tsv")
        ):
            ...
            contig_mappings["class"] = contig_mappings["category"].apply(
                lambda x: (
                    "singlerecovery"
                    if x in [col for col, c in settings["contig_classes"].items() if c["singlerecovery"]]
                    else "missed"
                )
            )
...
def getdata_gene_recovery(fp_orb_basedir: str, settings, full_gene_table=False, verbose=True) -> pd.DataFrame:
    ...
    recovered_genes = []
    for environment in tqdm(
        environments, disable=not verbose, desc="Compute sets of unique/shared/core genes for gene recovery plot"
    ):
        ...
        upset.loc["shared"] = features[
            (features.sum(axis=1) < len(features.columns)) & (features.sum(axis=1) > 1)
        ].shape[0]
        # total number genes
...
def getdata_runtime_memory(fp_caviar_basedir: str, settings, verbose=True):
    ...
    data = []
    for fp_time in tqdm(
        glob("%s/**/*_time_log.txt" % fp_caviar_basedir, recursive=True),
        disable=not verbose,
        desc="Compiling data for runtime/memory footprint plot",
    ):
    ...
    cmb_time = (
        data[data["type"] == "User time (seconds): "].set_index(["environment", "assembler"])["value"]
        + data[data["type"] == "System time (seconds): "].set_index(["environment", "assembler"])["value"]
    ).reset_index()
    cmb_time["type"] = "CPU time (seconds)"

    timemem = pd.concat([data, cmb_time])
    timemem = timemem[
        timemem["assembler"] != "convert_to_fasta"
    ]  # this is a pre-processing for some of the assembler as they not always except fastQ.gz
    ...
    for field in fields_to_collect + ["CPU time (seconds)"]:
        ...
        if field == "Maximum resident set size (kbytes): ":
            fct_combine = np.max
        combined = (
            timemem.set_index(["environment", "assembler", "type"])
            .loc[:, ["velveth", "velvetg", assembler], field, :]
            .reset_index()
            .groupby("environment")["value"]
            .apply(fct_combine)
            .to_frame()
            .reset_index()
        )
    ...
    timemem = pd.concat(
        [
            ...
            pd.concat(oases_combined),
        ]  # combined oases results
    )
    # pretty assembler names
...
def getdata_DEgenes(fp_orb_basedir: str, settings, verbose=True):
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for DE gene plot"):
        ...
        for fp_category in sorted(glob(join(fp_orb_basedir, environment, "deseq2", "*_DESeq2_full_table.tsv"))):
            ...
            conv = (
                recovered_contigs[environment][assembler]
                .merge(truth, left_on="gene_name", right_index=True, how="outer")
                .merge(prediction, left_on="Query sequence name", right_index=True, how="outer")
                .groupby("gene_name")
                .head(1)
                .fillna(False)
                .groupby(["truth", "prediction"])
                .size()
            )
    ...
    for environment in confusion.index.levels[0]:
        ...
        order = list(
            reversed(
                pd.pivot_table(
                    data=confusion.loc[environment, :],
                    index="assembler",
                    columns="class",
                    values="num_genes",
                    aggfunc="sum",
                )
                .sort_values(by=["True Positive", "False Positive", "False Negative"], ascending=[False, True, True])
                .index
            )
        )
...
def getdata_DEgenes_mod(fp_orb_basedir: str, settings, verbose=True, sortF1=False):
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for DE gene plot"):
        for fp_confusion in sorted(
            glob(join(fp_orb_basedir, environment, "calculatedgeconfusiondeseq2", "*_linear_cm.tsv"))
        ):
    ...
    for environment in confusion.index.levels[0]:
        cfg = pd.pivot_table(
            data=confusion.loc[environment, :], index="assembler", columns="class", values="num_genes", aggfunc="sum"
        )
        # compute F1 score: https://en.wikipedia.org/wiki/F-score#Definition
        cfg["F1"] = (
            2 * cfg["True Positive"] / (2 * cfg["True Positive"] + cfg["False Positive"] + cfg["False Negative"])
        )
        if sortF1:
            ...
            order = list(
                reversed(
                    cfg.sort_values(
                        by=["True Positive", "False Positive", "False Negative"], ascending=[False, True, True]
                    ).index
                )
            )
        confusion.loc[confusion.loc[environment, order, :].index, "rank"] = [
...
def getdata_DEorthogroups(fp_orb_basedir: str, fp_marbel_basedir: str, fp_ogtruth_basedir: str, settings, verbose=True):
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for DE orthogroup plot"):
        ...
        genes = pd.read_csv(
            join(fp_marbel_basedir, "%s_microbiome" % environment, "summary", "gene_summary.csv"), sep=","
        ).set_index("gene_name")
        ...
        for fp_assembler in sorted(
            glob(join(fp_ogtruth_basedir, environment, "%s_*_DESeq2_full_table.tsv" % environment))
        ):
            ...
            conv = (
                recovered_contigs[environment][assembler]
                .merge(genes[["orthogroup"]], left_on="gene_name", right_index=True, how="left")
                .merge(truth, left_on="orthogroup", right_index=True, how="outer")
                .merge(prediction, left_on="orthogroup", right_index=True, how="outer")
                .groupby("orthogroup")
                .head(1)
                .fillna(False)
                .groupby(["truth", "prediction"])
...
def getdata_DEorthogroups_mod(fp_orb_basedir: str, settings, verbose=True, sortF1=False):
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for DE orthogroups plot"):
        for fp_confusion in sorted(
            glob(join(fp_orb_basedir, environment, "calculateogconfusiondeseq2", "*_linear_cm.tsv"))
        ):
...
def getdata_DEvennOrtho(fp_orb_basedir: str, fp_marbel_basedir: str, settings, verbose=True) -> pd.DataFrame:
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for DE Venn diagram"):
        ...
        genes = genes.merge(
            ...
            .size()
            .apply(lambda x: "single_gene_OG" if x == 1 else "multi_gene_OG")
            .rename("OGsize"),
            left_on="orthogroup",
            right_index=True,
            how="left",
        )
        truth = truth.merge(genes[["orthogroup", "OGsize"]], left_index=True, right_index=True, how="left")
        ...
        truth = truth.merge(
            truthOG, left_on="orthogroup", right_index=True, how="left", suffixes=("_gene", "_orthogroup")
        )
...
def getdata_rnaquast(fp_orb_basedir: str, fp_quast_basedir: str, settings, verbose=True) -> pd.DataFrame:
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compile RNAquast data"):
        ...
        quast.columns = list(
            map(lambda x: settings["labels"]["assemblers"].get(x.split("_contigs")[0], x), quast.columns)
        )
...
def getdata_detonate(fp_orb_basedir, fp_detonate_basedir: str, settings, verbose=True) -> pd.DataFrame:
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compile Detonate data"):
        detonate = pd.read_csv(
            join(fp_detonate_basedir, "%s_detonate_scores_merged.tsv" % environment), sep="\t", index_col=0
        )
    ...
    # compute score_KC
    data = (data.loc[:, :, "weighted_kmer_recall"] - data.loc[:, :, "inverse_compression_rate"]).rename(
        columns={"score": "score_{KC}"}
    )
...
def getdata_block_recovery(fp_orb_basedir: str, fp_marbel_basedir: str, sequence_file: str, settings, verbose=True):
    ...
    for i, environment in enumerate(environments):  # , disable=not verbose, desc=''):
        ...
        blocks = pd.read_csv(
            join(fp_marbel_basedir, "%s_microbiome" % environment, "summary", "blocks.bed"),
            sep="\t",
            header=None,
            names=["gene", "start", "end", "block_name", "read_count"],
        )
        if verbose:
        ...
        genes_to_og = (
            pd.read_csv(join(fp_marbel_basedir, "%s_microbiome" % environment, "summary", "gene_summary.csv"), sep=",")
            .set_index("gene_name")["orthogroup"]
            .to_dict()
        )
        if verbose:
            print("done.", file=sys.stderr)
        ...
        og_means = calculate_og_seq_ident(
            join(fp_marbel_basedir, "%s_microbiome" % environment, "summary", "gene_summary.csv"),
            sequence_file,
            environment,
        )
        if verbose:
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for gene recovery plot"):
        ...
        for fp_n_stats in glob(join(fp_orb_basedir, environment, "minimap2classification", "*_contig_n_stats.tsv")):
            ...
            categories = pd.read_csv(
                join(fp_orb_basedir, environment, "categorizecontigs", f"{assembler}_contigs_categorised.tsv"),
                index_col=0,
                sep="\t",
            )
            ...
            categories["single_recovery"] = categories["category"].apply(
                lambda x: (
                    "singlerecovery"
                    if x in [col for col, c in settings["contig_classes"].items() if c["singlerecovery"]]
                    else "missed"
            ...
            categories["class"] = categories["category"].apply(
                lambda x: (
                    "recovered"
                    if x in [col for col, c in settings["contig_classes"].items() if c["class"] == "good"]
                    else "missed"
                )
            )
...
def filter_for_assembler_with_ns(fp_orb_basedir: str, settings, verbose=True):
    ...
    for environment in tqdm(environments, disable=not verbose, desc="Compiling data for n plot"):
        ...
        n_run_data["max_n_longer_read_length"] = (n_run_data["max_N_run"] > settings["read_length"]).map(
            {True: "> read length", False: "<= read length"}
        )
...
def get_genome_accession_map(fp_genomes: str):
    ...
    accessionmap = accessionmap.set_index("refseqacc_noversion").rename(
        columns={"refseqAccession": "refseqAccession_versioned"}
    )
...
def get_blockpos_ingenes(fp_genomes: str, fp_marbel_basedir: str, env: str, accessionmap):
    ...
    summary = summary.merge(
        accessionmap[["assemblyAccession", "refseqAccession_versioned"]],
        left_on="anchor_refseqAccession",
        right_index=True,
        how="left",
    )
    # obtain genomic positions of genes
    annotations = []
    for acc_assembly in tqdm(
        summary["assemblyAccession"].unique(), "collect block-coordinates and species annotations for %s" % env
    ):
        ...
        annot["locus_tag"] = annot[8].apply(
            lambda x: [entry.split("=")[-1] for entry in x.split(";") if entry.split("=")[0] == "locus_tag"][0]
        )
    ...
    summary = summary.merge(
        annotations[[3, 4, 6, "organism"]].reset_index(), left_on="gene_name", right_on="locus_tag", how="left"
    ).rename(columns={0: "assembly_accession", 3: "gene_genomic_start", 4: "gene_genomic_stop", 6: "strand"})
...
def get_genomic_hit_positons(fp_orb_basedir: str, fp_marbel_basedir: str, env: str, assembler: str, summary):
    mapping = pd.read_csv(
        "%s/%s/minimap2/%s_mapping.tsv" % (fp_orb_basedir, env, assembler), sep="\t", header=None
    ).rename(
        columns={
    ...
    blocks = pd.read_csv(
        join(fp_marbel_basedir, "%s_microbiome" % env, "summary", "blocks.bed"),
        sep="\t",
        header=None,
        names=["gene_name", "start", "stop", "block", "unknown"],
    )

    # add relative positions of blocks within genes
    mapping = mapping.merge(
        blocks.set_index("block")[["start", "stop"]].rename(
            columns={"start": "rel_block_start", "stop": "rel_block_stop"}
        ),
        left_on="Target sequence name",
        right_index=True,
        how="left",
    )
    # add genomic gene positions
    mapping = mapping.merge(
        summary.set_index("gene_name")[
            [
                "gene_genomic_start",
                "gene_genomic_stop",
                "strand",
    ...
    mapping["block_genomic_stop"] = mapping["block_genomic_start"] + (
        mapping["rel_block_stop"] - mapping["rel_block_start"] + 1
    )
