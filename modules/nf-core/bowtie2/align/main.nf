process BOWTIE2_ALIGN {
    ...
    output:
    tuple val(meta), path("${meta2.id}_bowtie2/*.{bam,sam}"), emit: aligned
    tuple val(meta), path("${meta2.id}_bowtie2/*.log")      , emit: log
    tuple val(meta), path("${meta2.id}_bowtie2/*fastq.gz")  , emit: fastq, optional:true
    path  "${meta2.id}_bowtie2/versions.yml"                , emit: versions
    ...
    """
    mkdir ${meta2.id}_bowtie2
    mv ${index} ${meta2.id}_bowtie2
    mv ./*.fast* ${meta2.id}_bowtie2
    cd ${meta2.id}_bowtie2
