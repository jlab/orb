...
summary_genes = pd.DataFrame(
    [summary_df["origin_gene_nunique"].describe(), summary_df["orthogroup_nunique"].describe()]
).T

summary_genes.columns = [f"{assembler_name}_{col}" for col in summary_genes.columns]

summary_mapping_stats = pd.DataFrame(
    [number_of_genes_with_one_read_mapped, number_of_reads_with_duplicated_mapping],
    columns=[f"{assembler_name}_mapping_stats"],
