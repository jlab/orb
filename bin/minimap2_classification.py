...
def deduplicate_by_key(df, group_key, seed):
    ...
    minimap_mapping["cumulative_score"] = (
        5 * (minimap_mapping[max_cols[0]] == minimap_mapping[new_max_cols[0]])
        + 3 * (minimap_mapping[max_cols[1]] == minimap_mapping[new_max_cols[1]])
        + 1 * (minimap_mapping[min_col[0]] == minimap_mapping[new_min_col[0]])
    )
    ...
    minimap_mapping["cooptimal_hits"] = minimap_mapping["cooptimal_hits"].astype("boolean").fillna(False).astype(bool)
    cooptimal_hits_selection_idx = (
        minimap_mapping[minimap_mapping["cooptimal_hits"]].groupby(group).sample(n=1, random_state=seed).index
    )

    minimap_mapping.loc[cooptimal_hits_selection_idx, "selected_cooptimal_hit"] = True

    minimap_mapping["selected_cooptimal_hit"] = (
        minimap_mapping["selected_cooptimal_hit"].astype("boolean").fillna(False).astype(bool)
    )
    selection_idx = minimap_mapping[
        minimap_mapping["optimal_hit"]
        & (~minimap_mapping["cooptimal_hits"] | minimap_mapping["selected_cooptimal_hit"])
    ].index
...
minimap_mapping = pd.read_csv(minimap_path, sep="\t", header=None, names=mapping_col_names, dtype={"contig": str})

minimap_overlap_df = pd.read_csv(minimap_overlap_path, sep="\t", dtype={"contig": str})
...
minimap_mapping["correctly_mapped_bases"] = (
    minimap_mapping["Number of matching bases in the mapping"] / minimap_mapping["Target sequence length"]
)
...
minimap_mapping["covered_reference"] = (
    minimap_mapping["Number bases, including gaps, in the mapping"] / minimap_mapping["Target sequence length"]
)
...
minimap_mapping = minimap_mapping[
    (minimap_mapping["covered_reference"] <= 1.05) & (minimap_mapping["covered_reference"] >= 0.95)
]
...
minimap_mapping["block_size"] = minimap_mapping.groupby(["block_id"]).transform("size")

minimap_mapping["contig_size"] = minimap_mapping.groupby(["contig"]).transform("size")

minimap_mapping = minimap_mapping.sort_values(
    by=["Mapping quality", "Number of matching bases in the mapping", "Query sequence length"],
...
minimap_mapping["assigned"] = minimap_mapping["assigned"].astype("boolean").fillna(False).astype(bool)
...
minimap_mapping["blocks_aggregated"] = minimap_mapping.groupby("contig")["block_id"].transform(
    lambda x: [list(x)] * len(x)
)

minimap_mapping["gene"] = minimap_mapping["block_id"].str.replace(r"^((?:[^_]+_){1}[^_]+).*", r"\1", regex=True)

minimap_mapping["genes_aggregated"] = minimap_mapping.groupby("contig")["gene"].transform(lambda x: [list(x)] * len(x))
...
minimap_mapping["ogs_aggregated"] = minimap_mapping.groupby("contig")["og"].transform(lambda x: [list(x)] * len(x))
...
block_is_super_recovered = minimap_mapping[
    minimap_mapping["assigned"] & (minimap_mapping["is_joined"]) & (minimap_mapping["is_one_gene"])
]["blocks_aggregated"].explode()
...
minimap_mapping["is_block_super_recovered_or_recovered"] = (
    minimap_mapping["is_block_recovered"] | minimap_mapping["is_block_super_recovered"]
)

# calculate blocks but subtract the blocks already assigned

super_recovered_amount_of_blocks = (
    minimap_mapping[minimap_mapping["assigned"] & (minimap_mapping["is_joined"]) & (minimap_mapping["is_one_gene"])][
        "contig_size"
    ].sum()
    - (
        minimap_mapping["assigned"]
        & (minimap_mapping["is_joined"])
        & ~(minimap_mapping["is_one_gene"])
        & (minimap_mapping["is_one_og"])
        & minimap_mapping["is_block_recovered"]
    ).sum()
)

og_recovered_amount_of_blocks = (
    minimap_mapping[
        minimap_mapping["assigned"]
        & (minimap_mapping["is_joined"])
        & ~(minimap_mapping["is_one_gene"])
...
duplicated_non_chimeric = dedupped_rest[(dedupped_rest["is_duplicated"] & ~dedupped_rest["is_joined"])]

duplicated_chimeric = dedupped_rest[(dedupped_rest["is_duplicated"] & dedupped_rest["is_joined"])]

minimap_mapping.loc[(minimap_mapping["assigned"] & (~minimap_mapping["is_joined"])), "category"] = (
...
minimap_mapping.loc[(dedupped_rest[dedupped_rest["is_duplicated"] & ~dedupped_rest["is_joined"]]).index, "category"] = (
...
minimap_mapping.dropna(subset=["category"], inplace=True)
...
contigs_n_df = pd.merge(minimap_mapping, ns_per_contig_df, how="left", right_on="contig_id", left_on="contig")

recovered_with_n = (
    minimap_mapping["category"].isin(
        ["minimap2_single_recovered", "minimap2_merged_recovered", "minimap2_orthologous_recovered"]
    )
    & (contigs_n_df["total_Ns"] > 0)
).sum()

chimeric_with_n = ((minimap_mapping["category"] == "minimap2_chimeric") & (contigs_n_df["total_Ns"] > 0)).sum()

minimap_mapping[["contig", "block_id", "category", "blocks_aggregated"]].to_csv(
    f"{prefix}_minimap2_categories.tsv", sep="\t"
)
...
scores = pd.Series(
    [super_recovered_amount_of_blocks, og_recovered_amount_of_blocks, recovered_with_n, chimeric_with_n],
    index=[
        "minimap2_merged_recovered_blocks",
        "minimap2_orthologous_recovered_blocks",
        "minimap2_recovered_with_n",
        "minimap2_chimeric_with_n",
    ],
)

categories.add(scores, fill_value=0).sort_index().to_csv(
    f"{prefix}_minimap2_category_counts.tsv", sep="\t", header=[prefix]
)
