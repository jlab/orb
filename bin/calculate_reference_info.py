...
pd.DataFrame({"Chimeric Blocks Lengths": chimeric_blocks["length"]}).to_csv(
    f"{prefix}_chimeric_block_lengths.tsv", sep="\t", index=False
)
...
chim_blocks_summary = pd.concat(
    [chimeric_blocks["length"].describe(), pd.Series({"total_length": total_chimeric_block_length})]
)

pd.DataFrame({"CDS": cds_summary, "Blocks": blocks_summary, "Chimeric Blocks": chim_blocks_summary}).to_csv(
    f"{prefix}_reference_stats.tsv", sep="\t"
)
