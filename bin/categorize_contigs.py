...
assembler_mapping = pl.read_csv(assembler_mapping_fl, separator="\t", has_header=False, new_columns=["contigs", "start", "end", "read_id"])
...
# Map gene_og_dict
assembler_mapping = assembler_mapping.with_columns([
    pl.col("origin_gene").map_elements(lambda x: gene_og_dict.get(x, None),return_dtype=pl.String).alias("origin_orthogroup")
])
...
single_mapped_contigs = contig_aggregation.filter(
    pl.col("origin_gene_nunique") == 1
)["contigs"].to_list() 
...
multi_mapped_contigs_multi_og = multi_mapped.filter(
    pl.col("origin_orthogroup_nunique") != 1
)["contigs"].to_list()

# multimapped but single og
multi_mapped_contigs_single_og = multi_mapped.filter(
    pl.col("origin_orthogroup_nunique") == 1
)["contigs"].to_list()
