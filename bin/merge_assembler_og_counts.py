...
assembler_with_og = (
    assembler_counts.with_columns(
        pl.col("Name").cast(pl.Utf8),
    )
    .with_columns(
        pl.col("Name")
        .map_elements(lambda x: assembler_mapping_dict.get(str(x), None), return_dtype=pl.Utf8)
        .alias("gene_block_name"),
    )
    .with_columns(pl.col("gene_block_name").str.replace(r"_block\d+$", "", literal=False).alias("gene_name"))
    .with_columns(
        pl.col("gene_name")
        .map_elements(lambda x: gene_to_og.get(str(x), None), return_dtype=pl.Utf8)
        .alias("orthogroups")
    )
    .group_by("orthogroups")
    .agg((pl.col(assembler_samples).sum()))
)
